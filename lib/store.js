...
function hasPostgres() {
  return Boolean(process.env.DATABASE_URL) && !postgresDisabled;
}
...
function postgresSsl() {
  ...
  if (/sslmode=require|neon\.tech|supabase\.|amazonaws\.|render\.com/i.test(url)) {
...
async function getPool() {
  ...
  if (!schemaReady) {
    let lastError;
    const maxAttempts = process.env.DATABASE_URL ? 12 : 3;
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      try {
        ...
        `);
        await pool.query("ALTER TABLE history ADD COLUMN IF NOT EXISTS credit BOOLEAN DEFAULT FALSE");
        await pool.query("ALTER TABLE history ADD COLUMN IF NOT EXISTS installments INTEGER DEFAULT 0");
        schemaReady = true;
        console.log("Postgres connected.");
        await migrateFileIfEmpty();
        break;
      } catch (error) {
        lastError = error;
    ...
    if (!schemaReady) {
      if (process.env.DATABASE_URL && process.env.NODE_ENV === "production") {
        console.error("Postgres unavailable:", lastError && lastError.message);
        throw lastError;
      }
      console.error("Postgres unavailable, using file store:", lastError && lastError.message);
      try {
        await pool.end();
      } catch (error) {
        // ignore
...
async function migrateFileIfEmpty() {
  ...
  if (!file.users.length && !file.history.length) return;

  for (const user of file.users) {
    await pool.query(
      "INSERT INTO users (id, username, password_hash, created_at) VALUES ($1, $2, $3, $4) ON CONFLICT (id) DO NOTHING",
      [user.id, user.username, user.passwordHash, user.createdAt || new Date().toISOString()]
    );
  }

  for (const item of file.history) {
    await pool.query(
      `INSERT INTO history
  ...
  console.log("Existing file data copied to Postgres.");
}
...
async function addHistory(userId, entry) {
  const item = {
    ...
    credit: Boolean(entry.credit),
    installments: Number(entry.installments) || 0,
  };
  ...
  if (hasPostgres()) {
    ...
    await db.query(
      `INSERT INTO history
        (id, user_id, name, phone, phone_digits, product, message, date, date_iso, credit, installments)
...
function storageMode() {
  if (process.env.DATABASE_URL && !postgresDisabled) return "postgres";
  return "file";
}
